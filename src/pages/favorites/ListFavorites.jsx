...
const ListFavorites = ({ data }) => {
  if (data && data.length < 1) {
    return (
      <div className="flex items-center justify-center w-full h-screen">
        <h1 className="text-lg font-bold animate-pulse">TYour favorites list is empty</h1>
      </div>
    );
  }
  return (
    <div className="w-full min-h-screen">
      <h1 className="py-5 text-2xl md:text-4xl font-bold italic font-['Bebas_Neue']">
        Your Favorite Series ({data.filter((el) => el.media_type.toLowerCase() === "tv").length})
      </h1>
      {data.filter((el) => el.media_type.toLowerCase() === "tv").length > 0 ? (
        ...
        <div className="flex items-center justify-center h-72">
          <h1 className="text-lg font-bold animate-pulse">favorite series list is empty</h1>
        </div>
      )}

      <h1 className="py-5 text-2xl md:text-4xl font-bold italic font-['Bebas_Neue']">
        Your Favorite Movies ({data.filter((el) => el.media_type.toLowerCase() === "movie").length})
      </h1>
      {data.filter((el) => el.media_type.toLowerCase() === "movie").length > 0 ? (
        ...
        <div className="flex items-center justify-center h-72">
          <h1 className="text-lg font-bold animate-pulse">Your favorite movies list is empty</h1>
        </div>
      )}
    </div>
  );
